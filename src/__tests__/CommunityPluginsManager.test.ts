...
describe('CommunityPluginsManager - Git Push & Admonition', () => {
  ...
  describe('formatTable', () => {
    ...
    it('should trigger table format command and return success message', () => {
      ...
      (mockPlugin.app.workspace.getActiveViewOfType as any).mockReturnValue(mockActiveView);
      (mockPlugin.app as any).commands = { executeCommandById: mockExecuteCommand };

      const result = manager.formatTable();
      expect(mockExecuteCommand).toHaveBeenCalledWith('table-editor-obsidian:format-table');
      expect(result).toBe('Triggered Advanced Tables formatting in **TableNote**.');
    });
    ...
    it('should return failure message if command execution fails or returns false', () => {
      ...
      const result = manager.formatTable();
      expect(mockExecuteCommand).toHaveBeenCalledWith('table-editor-obsidian:format-table');
      expect(result).toBe('Failed to trigger Advanced Tables formatting. Make sure the plugin is enabled and your cursor is inside a table.');
    });
  });
});
