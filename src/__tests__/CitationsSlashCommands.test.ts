...
vi.mock('obsidian', () => ({
  TFile: class TFile {
    ...
    path = '';
    basename = '';
    constructor(path: string, ext = 'md') {
      this.path = path;
      this.extension = ext;
...
describe('SlashCommands execution', () => {
  ...
  beforeEach(() => {
    mockPlugin = {
      ...
      tagManager: {
        suggestTagsForContent: vi.fn().mockReturnValue([{ tag: '#academic', confidence: 0.85 }]),
        applyTagsToNote: vi.fn().mockResolvedValue(undefined)
      },
      templateManager: {
        loadTemplates: vi.fn().mockResolvedValue([{ id: 'coach', name: 'Coach', prompt: 'Be a coach', icon: '✍️', description: 'Coaching' }]),
  ...
  describe('/cite style command', () => {
    ...
    it('should return error for invalid style', async () => {
      const citeCmd = getSlashCommands().find((c) => c.name === 'cite');
      const result = await citeCmd!.execute(mockPlugin as Plugin, 'style invalid');
  ...
  describe('/annotations command', () => {
    it('should extract annotations for valid PDF path', async () => {
      ...
      const mockFile = new TFile('papers/doc.pdf', 'pdf');
      ...
      mockPlugin.pdfAnnotationManager.formatAnnotationsMarkdown.mockReturnValue('Formatted Annotations');

      const result = await annotCmd!.execute(mockPlugin as Plugin, 'papers/doc.pdf');
      expect(result).toBe('Formatted Annotations');
      expect(mockPlugin.pdfAnnotationManager.extractAnnotations).toHaveBeenCalledWith(mockFile);
    });
    ...
    it('should return error for non-existent file', async () => {
      ...
      const result = await annotCmd!.execute(mockPlugin as Plugin, 'missing.pdf');
  ...
  describe('/context command', () => {
    it('should summarize active context items', async () => {
      ...
      mockPlugin.app.workspace.getLeavesOfType.mockReturnValue([{ view: mockView }]);
      mockPlugin.app.vault.getAbstractFileByPath.mockReturnValue({
        path: 'notes/test.md'
      });
      mockPlugin.app.vault.read.mockResolvedValue('Hello World! This note has words.');
  ...
  describe('/tags command', () => {
    it('should suggest tags', async () => {
      ...
      const { TFile } = await import('obsidian');
      const mockFile = new TFile('notes/test.md');
    ...
    it('should apply tags', async () => {
      ...
      const result = await tagsCmd!.execute(mockPlugin as Plugin, 'apply academic study');
      expect(result).toContain('Applied tags: **academic**, **study**');
      expect(mockPlugin.tagManager.applyTagsToNote).toHaveBeenCalledWith(mockFile, ['academic', 'study']);
    });
  });
  ...
  describe('/template command', () => {
    ...
    it('should save template', async () => {
      ...
      const mockView = {
        activeMessages: [
          { role: 'user', content: 'Design a system prompt' }
        ]
      };
      mockPlugin.app.workspace.getLeavesOfType.mockReturnValue([{ view: mockView }]);

      const result = await tplCmd!.execute(mockPlugin as Plugin, 'save my-coach');
      expect(result).toContain('Template **my-coach** saved successfully.');
      expect(mockPlugin.templateManager.saveTemplate).toHaveBeenCalledWith('my-coach', 'Design a system prompt');
    });
  });
  ...
  describe('/pdf command', () => {
    it('should extract page text', async () => {
      ...
      const result = await pdfCmd!.execute(mockPlugin as Plugin, 'page papers/doc.pdf 1');
    ...
    it('should extract metadata', async () => {
      ...
      const result = await pdfCmd!.execute(mockPlugin as Plugin, 'metadata papers/doc.pdf');
  ...
  describe('Feature Toggles', () => {
    ...
    it('should respect enableAnnotations = false for /pdf', async () => {
      ...
      const result = await pdfCmd!.execute(mockPlugin as Plugin, 'metadata paper.pdf');
      expect(result).toBe('PDF integrations are disabled in settings.');
    });
