...
const { FakeTFile } = vi.hoisted(() => {
  class FakeTFile {
    ...
    constructor(path: string) {
      ...
      this.parent = { path: path.includes('/') ? path.split('/').slice(0, -1).join('/') : '' };
...
const makeFile = (path: string) => new FakeTFile(path);

describe('NoteComposerManager', () => {
  describe('splitNoteAtHeading', () => {
    it('should return null when heading is not found', async () => {
      const plugin = makeMockPlugin();
      (plugin.app.vault.read as any).mockResolvedValue('# Introduction\n\nSome content.');
      const manager = new NoteComposerManager(plugin);
      const result = await manager.splitNoteAtHeading(makeFile('notes/my-note.md'), 'Nonexistent');
    ...
    it('should return null if destination note already exists', async () => {
      ...
      (plugin.app.vault.getAbstractFileByPath as any).mockReturnValue(makeFile('notes/target.md'));

      const manager = new NoteComposerManager(plugin);
      const result = await manager.splitNoteAtHeading(makeFile('notes/source.md'), 'Target');
      expect(result).toBeNull();
    });
  });
  ...
  describe('mergeNotes', () => {
    ...
    it('should return null if destination already exists', async () => {
      const plugin = makeMockPlugin();
      (plugin.app.vault.getAbstractFileByPath as any).mockReturnValue(makeFile('merged.md'));
      const manager = new NoteComposerManager(plugin);
      expect(await manager.mergeNotes([makeFile('a.md')], 'merged.md')).toBeNull();
    });
    ...
    it('should merge multiple notes and trash sources by default', async () => {
      ...
      expect(result).not.toBeNull();
      const [, createdContent] = (plugin.app.vault.create as any).mock.calls[0] as [string, string];
  ...
  describe('extractSelection', () => {
    it('should return null for invalid range (fromChar >= toChar)', async () => {
      ...
      const manager = new NoteComposerManager(plugin);
      expect(
        await manager.extractSelection(makeFile('test.md'), 10, 5, 'extracted.md')
      ).toBeNull();
    });

    it('should extract selected range to new file', async () => {
      const plugin = makeMockPlugin();
      (plugin.app.vault.read as any).mockResolvedValue('Hello World! This is extra.');
      (plugin.app.vault.getAbstractFileByPath as any).mockReturnValue(null);
      (plugin.app.vault.create as any).mockResolvedValue(makeFile('extracted.md'));

      const manager = new NoteComposerManager(plugin);
      const result = await manager.extractSelection(makeFile('source.md'), 6, 12, 'extracted.md');

      expect(result).not.toBeNull();
      const [createdPath, createdContent] = (plugin.app.vault.create as any).mock.calls[0] as [string, string];
