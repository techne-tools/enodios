...
function TagSuggestionComponent({ plugin, file, close }: TagSuggestionComponentProps) {
  ...
  const handleApply = async () => {
    const tagsToApply = Object.keys(selectedTags).filter((t) => selectedTags[t]);
    ...
    try {
      ...
      new Notice(`Successfully applied ${tagsToApply.length} tag(s) to ${file.basename}`);
      close();
    } catch (err) {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          <button className="btn btn-xs" onClick={handleSelectAll}>Select All</button>
          <button className="btn btn-xs" onClick={handleSelectNone}>Clear All</button>
        </div>
      </div>

      <div style={{ maxHeight: '300px', overflowY: 'auto', border: '1px solid var(--border-color)', borderRadius: '4px', padding: '5px' }}>
        {suggestions.map((s) => {
          ...
          return (
            <div
              key={s.tag}
              onClick={() => handleToggleSelect(s.tag)}
              style={{
                display: 'flex',
                alignItems: 'center',
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                <input
                  ...
                  type="checkbox"
                />
                <span style={{ fontWeight: '600', color: 'var(--text-normal)' }}>{s.tag}</span>
              </div>
              <span
                style={{
                  fontSize: '0.85em',
                  padding: '2px 6px',
                  borderRadius: '12px',
                  backgroundColor: pct >= 80 ? 'var(--text-success)' : pct >= 60 ? 'var(--text-accent)' : 'var(--background-modifier-border)',
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '10px' }}>
        ...
        <div style={{ display: 'flex', gap: '10px' }}>
          ...
          <button className="btn btn-primary" onClick={handleApply}>Apply Selected Tags</button>
        </div>
      </div>
    </div>
  );
