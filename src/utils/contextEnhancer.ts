 ...
 */
export async function getEnhancedNoteContext(
  plugin: Plugin,
  file: TFile,
): Promise<string> {
  ...
  try {
    ...
    const created = isNaN(ctime.getTime())
      ? new Date().toISOString()
      : ctime.toISOString();
    const modified = isNaN(mtime.getTime())
      ? new Date().toISOString()
      : mtime.toISOString();
    ...
    const backlinksStr =
      backlinks.length > 0
        ? backlinks
            .map((b) => `[[${b.sourcePath}]] (${String(b.linkCount)})`)
            .join(", ")
        : "None";
    ...
    const metadataBlock = [
      ...
      `Character Count: ${String(charCount)}`,
      `Tags: ${tags}`,
    ...
    plugin.debug.error(
      `Failed to generate enhanced context for ${file.path}`,
      err,
    );
...
export async function getFolderContext(
  ...
  summary += `Total Files: ${String(allFiles.length)}\n\n`;
  ...
  for (const file of allFiles) {
    ...
    const mtimeStr = new Date(file.stat.mtime)
      .toISOString()
      .substring(0, 16)
      .replace("T", " ");

    summary += `| ${file.path} | ${String(file.stat.size)} | ${mtimeStr} | ${tags} |\n`;
  ...
  if (embedNotes.length > 0) {
    summary += `#### Contents of Top ${String(embedNotes.length)} Notes in Folder:\n`;
