...
- [x] **v0.4.1-beta1 Hotfix (10 June 2026)**:
  - Fixed tool execution context lost when `showToolUse` is off — tool messages always tracked in chat history to prevent hallucinated "permission denied" agent reasoning
  - Fixed permission bubble now renders all Hermes ACP options correctly ("Allow once", "Allow for session", "Allow always", "Deny", "Deny always") using `option.name` from the protocol
  - Removed duplicate hardcoded Deny button — Hermes already sends deny options
  - Added compact backgrounded tool indicators when `showToolUse` is disabled
  - Extended `ChatMessage` interface with `isBackgrounded` field
  - Added `.hermes-tool-backgrounded` CSS style for minimal single-line tool indicators
