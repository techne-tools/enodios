...
- **Audit log shows useful information** — The audit trace panel now renders structured metadata (path, command, exit code, tool, permission, error) instead of raw JSON details.

### Bug Fixes

- **TemplateManager tests updated** — Tests still asserted the old `hermes/templates` path after the rename to `enodios/templates`; now aligned.
